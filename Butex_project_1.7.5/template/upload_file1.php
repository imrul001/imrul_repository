...
//    echo 'temp_file: '.($_FILES["file"]["tmp_name"]).'<br />';
            ...
            if (file_exists("./template/download_files/" . $_FILES["file"]["name"])) {
                ...
                $download_id= md5(createRandom_number());
                echo 'File Upload is Successfull. File Name:' . $_FILES["file"]["name"];
                $sql = "INSERT INTO `download_manager` (`title`, `file_name`, `date_of_upload`, `download_id`) VALUES('$title', '$file_name', '$date_of_upload', '$download_id')";
                @mysql_query($sql);
            }
        }
    } else {
