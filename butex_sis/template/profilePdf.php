    ...
    if (!empty($result)) {
        ...
        while ($row = mysql_fetch_array($result)) {
            ...
            if ($i == $num_row) {
                $current_y = $pdf->GetY();
                $current_x = $pdf->GetX();
                $pdf->MultiCell(50, $heightForDate, $row['date_time'], LTB, L);
...
//                $current_x = $pdf->GetX();
                ...
                $pdf->MultiCell(142, 5, "PUNISHMENT : ".$row['punishment'].". \nWARNING : ".$row['warning'], LTBR, L);
//                $pdf->Cell(31, 10, '', 0, 1, C);
            } else {
                $current_y = $pdf->GetY();
                $current_x = $pdf->GetX();

                $pdf->MultiCell(50, $heightForDate, $row['date_time'], LT, L);
